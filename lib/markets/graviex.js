...
function get_summary(coin, exchange, cb) {
  var summary = {};
  var url=base_url + '/tickers/' + coin.concat(exchange);
  request({"rejectUnauthorized": false, uri: url, json: true}, function (error, response, body) {
    if (error) {
      return cb(error, null);
    } else if (body.error !== true) {

      summary['ask'] = parseFloat(body['buy']).toFixed(9);
      summary['bid'] = parseFloat(body['sell']).toFixed(9);
      request({"rejectUnauthorized": false, uri: url , json: true}, function (error, response, body) {
        if (error) {
          return cb(error, null);
        } else if (body.error !== true) {
...
function get_trades(coin, exchange, cb) {
	  var req_url = base_url + '/trades_simple.json?market=' + coin.concat(exchange);
...
function get_orders(coin, exchange, cb) {
        var req_url = base_url + '/depth.json?market=' + coin.concat(exchange);
	request({ uri: req_url, json: true }, function (error, response, body) {
        if(error)
            return cb(error, null);
        else if (body.error !== true) {
