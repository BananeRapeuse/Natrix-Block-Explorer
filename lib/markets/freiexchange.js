  ...
  request({uri: req_url, json: true}, function (error, response, body) {
    if (error) {
     ...
     var pair = body[coin + '_' + exchange];
        ...
        summary['bid'] = parseFloat(pair[0].highestBuy).toFixed(8);
        summary['ask'] = parseFloat(pair[0].lowestSell).toFixed(8);
        summary['volume'] = parseFloat(pair[0].volume24h).toFixed(8);
        summary['volume_btc'] = parseFloat(pair[0].volume24h_btc).toFixed(8);
        summary['low'] = parseFloat(pair[0].high).toFixed(8);
        summary['high'] = parseFloat(pair[0].low).toFixed(8);
