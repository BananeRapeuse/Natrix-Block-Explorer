var mongoose = require('mongoose')
  ...
  , altilly = require('./markets/altilly')
...
function get_market_data(market, cb) {
  switch(market) {
    case 'bittrex':
      ...
      break;
	case 'freiexchange':
       freiexchange.get_data(settings.markets.coin.toUpperCase(), settings.markets.exchange.toUpperCase(), function(err, obj){
        return cb(err, obj);
      });
      break;  
	case 'altilly':
       altilly.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
        return cb(err, obj);
      });
      break;  
	case 'graviex':
